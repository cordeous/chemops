...
const FEATURES = [
  {
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 10V11" /></svg>,
  ...
  {
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>,
...
function DashboardMockup({ role }) {
  ...
  return (
    <svg viewBox="0 0 360 220" className="w-full rounded-xl" aria-hidden="true" style={{ filter: 'drop-shadow(0 4px 24px rgba(0,0,0,0.08))' }}>
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      {/* NAV */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100" aria-label="Main">
      ...
      <section className="relative pt-36 pb-24 px-6 overflow-hidden"
        style={{ background: 'linear-gradient(160deg, #0f1d3a 0%, #1a2e5a 60%, #243d73 100%)' }}>
        <div className="relative max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <div className="inline-block bg-[#f07c1e]/15 border border-[#f07c1e]/30 rounded-full px-4 py-1.5 mb-6">
              <span className="text-[#f07c1e] text-xs font-semibold tracking-widest uppercase">Chemical Sales &amp; Billing Platform</span>
            </div>
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-white mb-6 leading-tight">
              Manage Chemicals.<br />
              <span className="text-[#f07c1e]">Stay Compliant.</span>
            </h1>
            <p className="text-lg text-white/70 max-w-2xl mx-auto mb-10 leading-relaxed">
              The integrated platform for chemical distributors — inventory, orders, invoices, compliance tracking, and role-based dashboards with real analytics.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <button
                onClick={() => navigate('/login')}
                aria-label="Sign in to explore ChemOps"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-gray-100">
            {STATS.map(s => (
              <div key={s.label} className="px-6 py-8 text-center">
                <div className="text-2xl font-bold text-[#1a2e5a]">{s.value}</div>
      ...
      <section id="features" className="py-24 px-6 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {FEATURES.map(feat => (
              <div key={feat.title} className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-200 group will-change-transform">
                <div className="w-9 h-9 rounded-lg flex items-center justify-center mb-4 bg-[#1a2e5a]/8 text-[#1a2e5a]">
                  {feat.icon}
                </div>
      ...
      <section id="dashboards" className="py-24 px-6 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-gray-500 max-w-xl mx-auto">Each role gets a custom view with the charts and actions that matter most to their workflow. Sign in with any demo account to explore.</p>
          </div>
            ...
            {ROLES.map(r => (
              <div key={r.role} className="rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 group">
                ...
                <div className="flex items-center gap-3 px-5 py-3 border-b border-gray-100 bg-gray-50">
                  ...
                  <div className="ml-auto text-xs text-gray-300 font-mono">{r.role.toLowerCase()}@chemops.com</div>
                </div>
                {/* SVG Mockup */}
      ...
      {/* CTA */}
      <section className="py-24 px-6" style={{ background: 'linear-gradient(160deg, #1a2e5a 0%, #243d73 100%)' }}>
        <div className="max-w-2xl mx-auto text-center">
          <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4">Ready to explore the platform?</h2>
          <p className="text-white/70 mb-10 text-lg">Sign in with any demo account and explore every feature from that role's perspective.</p>
          <button onClick={() => navigate('/login')} aria-label="Sign in to ChemOps" className="px-10 py-4 rounded-xl bg-[#f07c1e] text-white font-semibold text-lg hover:bg-[#d96b10] transition-all shadow-lg hover:-translate-y-0.5 will-change-transform">
            Sign In to Demo
          </button>
      ...
      <footer className="py-8 px-6 bg-[#0f1d3a] text-center">
        <div className="flex items-center justify-center gap-2 mb-3">
          <div className="w-6 h-6 rounded bg-[#1a2e5a] flex items-center justify-center">
            <svg className="w-3 h-3 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 10V11" /></svg>
          </div>
